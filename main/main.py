...
class Bootstrap:
    def __init__(self, study_idx, log):
        ...
        # Set up log file.
        if log == 1:
            so = se = open(os.environ['DIAGNOSTICS'] + '/Output_'+str(study_idx)+'.log', 'w', 0)
        ...
        self.optimise = Optimise(self.geometry, self.study_ids[study_idx], self.study_frames[study_idx], study_idx,
    ...
    def ref_error_evaluate(self, study_idx):
        ...
        file = os.environ['SIMULATIONS_OPENCMISS']+'/'+self.study_ids[study_idx]+'/ReferenceWall'
        true_ref = os.environ['EXTRAS'] + '/GenerateSyntheticData/'+self.study_ids[study_idx]+'/ReferenceWall'
        os.chdir(os.environ['SIMULATIONS_OPENCMISS']+'/'+self.study_ids[study_idx])
