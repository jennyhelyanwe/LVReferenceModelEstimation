...
class Geometry:
    def __init__(self, study_ids, study_frames, num_studies, synth_ds_num, bootstrap_itr):
        ...
        #self._generate_data_matrix_displacement_from_ds_synthetic()
        #self._generate_data_matrix_displacement_from_reference_synthetic()
        #self._generate_data_matrix_displacement_ds_ref_only()
        self._get_data_matrix_ds_to_ref()
        print 'GEOMETRY: Perform PCA...'
        #self._pca()
        self._pca_all_modes()
        print 'GEOMETRY: Project each study to PCA to get initial guess for scores...'
        #self._project_studies_onto_pca()
    ...
    def _generate_data_matrix_displacement_from_ds_synthetic(self):
    ...
    def _generate_data_matrix_displacement_from_reference_synthetic(self):
        # Convert all diastolic frames to unit scale factors for conversion into Bezier.ps
        self.X = []
        self.data_info = []
        self.reference_models = np.zeros([self.num_studies, 960])
        # Loop through studies and evaluate deformation data.
        for i in range(0, self.num_studies):
            ...
            ref_path = os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/'
            os.chdir(ref_path)
            ref_file = os.environ['EXTRAS'] + '/GenerateSyntheticData/' + self.study_ids[i] + '/ReferenceWall'
            # print ref_file
            unit_sf_file = ref_file + '_UnitSF'
            convert_arithmetic_to_unit_sf(ref_file, unit_sf_file)
            nodes = read_ipnode(unit_sf_file + '.ipnode')
            ...
            self.reference_models[i, :] = np.reshape(temp, [960])
            # Get all other frames and calculate displacement from reference models.
            for j in range(0, len(self.idx)):
                if j == 0:
                    file_name = os.environ['EXTRAS'] + '/GenerateSyntheticData/' + study_id + '/ReferenceWall'
                else:
                    file_name = os.environ['SYNTHETIC_DATA'] + '/' + study_id + '/LVInflation_' + str(self.idx[j])
                unit_sf_file = file_name + '_UnitSF'
                convert_arithmetic_to_unit_sf(file_name, unit_sf_file)
                nodes = read_ipnode(unit_sf_file + '.ipnode')
                dofs = ipnode_to_dofs_format(nodes)
                ...
                self.data_info.append([study_id, self.idx[j]])
        self.X = np.array(self.X)
        pickle.dump(self.X, open('data_matrix_ref_only.pl', 'w'))
        pickle.dump(self.data_info, open('data_info_ref_only.pl', 'w'))
        pickle.dump(self.reference_models, open('reference_models_ref_only.pl', 'w'))
    ...
    def _get_data_matrix(self):
        self.X = pickle.load(open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/data_matrix_ref_only.pl', 'r'))
        self.data_info = pickle.load(open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/data_info_ref_only.pl', 'r'))
        self.reference_models = pickle.load(
            open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/reference_models_ref_only.pl', 'r'))

    def _get_data_matrix_ds_to_ref(self):
        self.X = pickle.load(open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/data_matrix_ds_to_ref.pl', 'r'))
        self.data_info = pickle.load(open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/data_info_ds_to_ref.pl', 'r'))
        self.reference_models = pickle.load(
            open(os.environ['REFERENCE_MODELS'] + '/b' + str(self.bootstrap_itr) + '/reference_models_ds_to_ref.pl', 'r'))
    ...
    def _pca_all_modes(self):
        pca = decomposition.PCA(n_components=len(self.X)-1)
        pca.fit(self.X)
        self.pca_mean = pca.mean_
        self.pca_components = pca.components_.T
        self.pca_variance = pca.explained_variance_
        ...
        print 'Sum of PCA explained variance for 28 modes:'
        print self.sum_pca_explained_variance

    def _project_studies_onto_pca(self):
        self.scores = np.zeros((self.num_studies, len(self.X)-1))
        for study_idx in range(0, 28):
            idx = self.data_info.index([self.study_ids[study_idx], self.idx[self.synth_ds_num]])
            Y = self.X[idx, :]
    ...
    def reconstruct_reference_model(self, scores, study_idx):
        ...
        ref_file = os.environ['SIMULATIONS_OPENCMISS']+'/'+study_name+'/ReferenceWall'
